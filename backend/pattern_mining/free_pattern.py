...
def arguments_validity(pattern_function: PatternFunction, arguments: dict):
    ...
    all_arguments_variables_or_objects = all(type(x) in {ObjectArgument, ObjectVariableArgument} for x in arguments.values())
...
class FreePattern:
    ...
    __arguments: dict  # index -> ObjectArgument | ObjectVariableArgument
    ...
    def __get_argument_bindings(self):
        return [self.__arguments[i] for i in range(self.__patternFunction.arity)]
    ...
    def evaluate(self, ocel: OCEL, event):
        ...
        assert arguments_completeness(argument_bindings)
        object_ids = list(map(lambda arg: arg.objectId, argument_bindings))
        ground_pattern: GroundPattern = self.__patternFunction.bind(object_ids)
        ground_pattern.apply(ocel, event)
    ...
    def substitute(self, object_arg: ObjectArgument, object_variable_arg: ObjectVariableArgument):
        for i, arg in self.__arguments.items():
            if equals(arg, object_variable_arg):
                self.__arguments[i] = object_arg

    def copy(self):
        return FreePattern(self.__patternFunction, self.__arguments)


    def to_string(self):
        function_identifier = self.__patternFunction.to_string()
        argument_bindings = self.__get_argument_bindings()
        arg: Argument
